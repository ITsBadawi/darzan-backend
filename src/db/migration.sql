...
CREATE TABLE IF NOT EXISTS products (
  id             UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  product_number INT GENERATED BY DEFAULT AS IDENTITY UNIQUE,
  ...
  sale_type      TEXT NOT NULL DEFAULT 'both' CHECK (sale_type IN ('both', 'dozen', 'piece')),
  price_piece    INTEGER NOT NULL DEFAULT 0,
  price_dozen    INTEGER NOT NULL DEFAULT 0,
  min_piece_qty  INTEGER NOT NULL DEFAULT 1,
  min_dozen_qty  INTEGER NOT NULL DEFAULT 1,
...
-- ─── SUPPLIERS — public read active, admin/editor full access ─
...
CREATE POLICY "Admin/editor can insert suppliers"
  ON suppliers FOR INSERT
  WITH CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'editor'))
  );

CREATE POLICY "Admin/editor can update suppliers"
  ON suppliers FOR UPDATE
  USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'editor'))
  );

CREATE POLICY "Admin/editor can delete suppliers"
...
-- ═══════════════════════════════════════════════════════════════
-- 12. MIGRATION FOR EXISTING DATABASES (Safe to re-run)
-- ═══════════════════════════════════════════════════════════════
ALTER TABLE products ADD COLUMN IF NOT EXISTS sale_type TEXT DEFAULT 'both';
ALTER TABLE products ADD COLUMN IF NOT EXISTS price_piece INTEGER DEFAULT 0;
ALTER TABLE products ADD COLUMN IF NOT EXISTS price_dozen INTEGER DEFAULT 0;
ALTER TABLE products ADD COLUMN IF NOT EXISTS min_piece_qty INTEGER DEFAULT 1;
ALTER TABLE products ADD COLUMN IF NOT EXISTS min_dozen_qty INTEGER DEFAULT 1;

ALTER TABLE order_items ADD COLUMN IF NOT EXISTS unit_type TEXT DEFAULT 'piece';
ALTER TABLE order_items ADD COLUMN IF NOT EXISTS unit_name TEXT DEFAULT 'قطعة';
